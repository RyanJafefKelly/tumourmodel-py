...
#include <cassert>
#include <cmath>      // use <cmath> and std::abs
#include <limits>
#include <algorithm>
...
static inline double dist(double ax, double ay, double bx, double by) {
    double dx = ax - bx;
    double dy = ay - by;
    return dx * dx + dy * dy;
}

static inline double orient2d(double ax, double ay, double bx, double by, double cx, double cy) {
    return (ay - cy) * (bx - cx) - (ax - cx) * (by - cy);
}

static inline bool inCircle(double ax, double ay, double bx, double by, double cx, double cy, double px, double py) {
...
static inline double circumradius(double ax, double ay, double bx, double by, double cx, double cy) {
...
static inline void circumcenter(double ax, double ay,
                                double bx, double by,
                                double cx, double cy,
...
static inline void quicksort(int* ids, double* dists, int left, int right)
{
    if (right - left <= 10)
    {
...
public:
    ...
    Delaunator(double* coords, int n)
    {
        ...
        {
            int i = i1;
            double x = i1x, y = i1y;        // was int; must be double
